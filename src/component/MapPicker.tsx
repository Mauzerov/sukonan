...
export function MapPicker() {
    ...
    return (
        <>
            ...
            {!maps.length && <div className="map-picker__no-maps">
                No maps found. Create one in the editor.
                <Link to="/editor" className="button">Editor</Link>
            </div>}
            {maps.length > 0 && <div
                className="map-picker"
                style={{"--own-map-count": maps.length} as React.CSSProperties}>
                {maps && maps.map((map: string[], i: number) => {
                    ...
                    return (
                        <div key={i} className="map-picker-map">
                            <MapGrid
                                cellSize={"auto"}
                                key={i}
                                map={mapString}
                            ...
                            <div className="map-picker-map-buttons">
                                <button onClick={() => navigate(`/own/${i}`)}>Play</button>
                                <button onClick={() => navigate(`/editor/${i}`)}>Edit</button>
                                <button onClick={() => {
                                ...
                                }}><TrashCan style={{fill: "darkred"}} /></button>
                            </div>
                        </div>)
                })}
            </div>}
